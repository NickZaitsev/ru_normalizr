...
def convert_roman_words(text: str) -> str:
    words = {
        "в.": ("в.", "-й"), "век": ("век", "-й"), "века": ("века", "-го"), "веку": ("веку", "-му"),
        "веке": ("веке", "-м"), "веком": ("веком", "-м"), "веками": ("веками", "-ми"), "веках": ("веках", "-х"),
        "столетие": ("столетие", "-е"), "столетия": ("столетия", "-го"), "столетию": ("столетию", "-му"),
        "столетии": ("столетии", "-м"), "столетием": ("столетием", "-м"), "столетиях": ("столетиях", "-х"),
        "глава": ("глава", "-я"), "главы": ("главы", "-й"), "главе": ("главе", "-й"), "главу": ("главу", "-ю"),
        "главой": ("главой", "-й"), "главами": ("главами", "-ми"), "главах": ("главах", "-х"),
        "раздел": ("раздел", "-й"), "раздела": ("раздела", "-го"), "разделе": ("разделе", "-м"), "разделу": ("разделу", "-му"),
        "часть": ("часть", "-я"), "части": ("части", "-й"), "частью": ("частью", "-й"),
        "том": ("том", "-й"), "тома": ("тома", "-го"), "томе": ("томе", "-м"), "томом": ("томом", "-м"),
        "книга": ("книга", "-я"), "книги": ("книги", "-й"), "книге": ("книге", "-й"), "книгой": ("книгой", "-й"),
        "квартал": ("квартал", "-й"), "квартала": ("квартала", "-го"), "кварталу": ("кварталу", "-му"),
        "квартале": ("квартале", "-м"), "кварталом": ("кварталом", "-м"), "кв.": ("квартал", "-й"), "кв": ("квартал", "-й"),
        "в ": ("в ", ""),
    }
...
def convert_heading_roman_numerals(text: str) -> str:
    heading_words = ("глава", "главы", "часть", "части", "раздел", "раздела", "разделе", "том", "тома", "томе", "книга", "книги", "книге")
...
def normalize_cyrillic_roman(text: str) -> str:
    cyrillic_to_latin = {"Х": "X", "х": "X", "С": "C", "с": "C", "І": "I", "l": "I", "і": "I", "М": "M", "м": "M"}
